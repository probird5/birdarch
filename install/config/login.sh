...
if [ -d "/boot/loader/entries" ]; then # systemd-boot
  ...
  if ! grep -q splash /etc/cmdline.d/*.conf; then
    # Need splash, create the birdarch file
    echo "splash" | sudo tee -a /etc/cmdline.d/birdarch.conf
  fi
  if ! grep -q quiet /etc/cmdline.d/*.conf; then
    # Need quiet, create or append the birdarch file
    echo "quiet" | sudo tee -a /etc/cmdline.d/birdarch.conf
  fi
...
if [ "$(plymouth-set-default-theme)" != "birdarch" ]; then
  sudo cp -r "$HOME/.local/share/birdarch/default/plymouth" /usr/share/plymouth/themes/birdarch/
  sudo plymouth-set-default-theme -R birdarch
fi
...
if [ ! -f /etc/systemd/system/birdarch-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/birdarch-seamless-login.service
[Unit]
Description=Omarchy Seamless Auto-Login
Documentation=https://github.com/basecamp/birdarch
...
# Enable birdarch-seamless-login.service only if not already enabled
if ! systemctl is-enabled birdarch-seamless-login.service | grep -q enabled; then
  sudo systemctl enable birdarch-seamless-login.service
fi
